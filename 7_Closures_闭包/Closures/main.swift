...
// 闭包声明
let closureStatement: () -> Int
// 闭包实现
closureStatement = { () -> Int in
    return 0
}
// 闭包调用
print("闭包调用：\(String(describing: closureStatement)) 内容值:\(closureStatement())")
...
// 这里使用闭包的形式实现这个函数，这里是函数调用
// match 参数 a 参数， b 参数
printMathResult(mathFunction: { (a: Int, b: Int) -> Int in return a * b }, 11, 6)
...
func download(_ name: String, from: Server) -> Picture? {
    return Picture()
}
// MARK: 多个尾随闭包
...
// 调用， 第一个闭包是完成处理程序，该处理程序在成功下载后显示图片。第二个闭包是一个错误处理程序，向用户显示错误
...
// 无参有返回值 闭包
/// 自定义闭包
/// - Parameter content: 无参有返回值闭包
func customSomeClosure(content: () -> CustomTemp) {
    // 调用闭包
    let result = content()
...
// 调用闭包, 声明， 实现
customSomeClosure { () -> CustomTemp in return CustomTemp(name: "one") }
// 调用闭包, 声明， 实现
customSomeClosure { () -> CustomTemp in
    return CustomTemp(name: "two")
}
// 调用闭包
customSomeClosure(content: { return CustomTemp(name: "three") })
// 调用闭包
customSomeClosure {
    return CustomTemp(name: "Four")
}
