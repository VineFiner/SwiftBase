...
struct DynamicStruct {
    ...
    subscript(member: String) -> String {
        return "\(multiplier)" + member
    }
}

let s = DynamicStruct(multiplier: 3)
// Value of type 'DynamicStruct' has no member 'test'
//s.test
print("This is dynamic:\(s["test"])---")
...
let subName: String = subTitan["name"]
print("This is sub:\(subName)")
// 这里需要注明取得的类型
let titan = Titan()
...
struct StaticTitan {
    // 这里是下标
    static subscript(member: String) -> String {
        let properties = ["name": "名字",
                          "city": "Hang"]
        return properties[member, default: "默认：0"]
    }
    // 这里是类型 动态成员查找
    static subscript(dynamicMember member: String) -> String {
        let properties = ["name": "动态名字",
                          "city": "Hang"]
        return properties[member, default: "默认：0"]
    }

}
print("static dynamic:\(StaticTitan["name"])\n\n")
print("static dynamic:\(StaticTitan.name)\n\n")
